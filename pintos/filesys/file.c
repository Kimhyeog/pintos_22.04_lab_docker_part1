...
struct file
{
	...
	bool deny_write;
	int dupCount; // [추가] 참조 카운트         /* Has file_deny_write() been called? */
};
...
struct file *file_open(struct inode *inode)
{
	...
	if (inode != NULL && file != NULL)
	{
		...
		file->dupCount = 1; // [수정] 생성 시 1로 초기화
		return file;
	}
	// ... (실패 처리 로직은 그대로)
	inode_close(inode);
...
struct file *
file_duplicate_shallow(struct file *file)
{
	if (file == NULL)
		return NULL;

	file->dupCount++;
	return file; // 주소값 그대로 반환
}
...
void file_close(struct file *file)
{
	if (file != NULL)
	{
		file->dupCount--; // [수정] 참조 카운트 감소

		// 참조 카운트가 0일 때만 진짜로 닫음
		if (file->dupCount == 0)
		{
